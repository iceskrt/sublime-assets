...
if not BBOT then
	if game.PlaceId == 292439477 then
		BBOT = { username = "cracker monkey", version = "0.0.1a", game = "Phantom Forces"}
	else
		-- Universal
		BBOT = { username = "cracker monkey", version = "3.5.1b", game = "Universal" }
	end
end

local menu

local MenuName = isfile("sublime/menuname.txt") and readfile("sublime/menuname.txt") or nil
...
do -- Files
	if not isfolder("sublime") then
		makefolder("sublime")
	end

	if not isfile("sublime/relations.bb") then
		writefile("sublime/relations.bb", "friends:\npriority:")
	end

    if not isfile("sublime/chatspam.bb") then
        writefile("sublime/chatspam.bb",
            [[BEACH BOAT ON TOP
...
beach boat does not miss]])
    ...
    if not isfile("sublime/killsay.bb") then
        writefile("sublime/killsay.bb",
...
[name] get better]])
    ...
    if not isfolder("sublime/".. BBOT.game) then
        makefolder("sublime/".. BBOT.game)
    end

    if not isfile("sublime/".. BBOT.game .. "/Default.bb") then
        writefile("sublime/".. BBOT.game .. "/Default.bb", "")
    end
end

do
    local customtxt = readfile("sublime/chatspam.bb")
    for s in customtxt:gmatch("[^\n]+") do
        table.insert(customChatSpam, s)
    end

    customtxt = readfile("sublime/killsay.bb")
    for s in customtxt:gmatch("[^\n]+") do
...
local function GetConfigs()
	local result = {}
	local directory = "sublime\\".. BBOT.game
...
local function UnpackRelations()
	local str = isfile("sublime/relations.bb") and readfile("sublime/relations.bb") or nil
	local final = {
		friends = {},
		priority = {},
	}
...
local function WriteRelations()
	...
	writefile("sublime/relations.bb", str)
end
CreateThread(function()
...
function menu.Initialize(menutable)
	...
	do
		...
		Draw:MenuBigText("sublime" .. " - " .. os.date("%b. %d, %Y"), true, false, 6, 6, bbmenu)
	...
	function menu:InputBeganMenu(key) --ANCHOR menu input --
		if key.KeyCode == (menu:GetVal("Settings", "Cheat Settings", "Custom Menu Key") and keyz[tonumber(menu.options["Settings"]["Cheat Settings"]["Custom Menu Key"][5][1].Value)] or Enum.KeyCode.Delete) and not loadingthing.Visible then
			cp.dragging_m = false
			cp.dragging_r = false
			cp.dragging_b = false
	...
	function menu.saveconfig()
		...
		writefile("sublime/".. BBOT.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb", figgy)
		CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.bb"!')
		UpdateConfigs()
	end

	function menu.loadconfig()
		local configname = "sublime/".. BBOT.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb"
	...
	local function buttonpressed(bp)
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			delfile("sublime/".. BBOT.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".bb")
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					local menutext = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") .. " - " .. os.date("%b. %d, %Y") or "sublime" .. " - " .. os.date("%b. %d, %Y")
					local menutext2 = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") or "sublime"
